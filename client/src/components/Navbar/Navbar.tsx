...
export const Navbar: FC = () => {
  ...
  const { resetConnectedUser, connectedUser } = useGlobalContext();
  ...
  const bottomMenuItems: MenuItem[] = [
    {
      ...
      icon: <LogoutIcon />,
      path: "/login",
      onClick: () => resetConnectedUser(),
    },
  ];

  const relevantMenuItems =
    connectedUser?.userLevel === 1
      ? EmployeeMainMenuItems
      : ManagerMainMenuItems;
          ...
          {bottomMenuItems.map((item, index) => {
            ...
            return (
              <ListItem key={item.text} disablePadding>
                <ListItemButton
                  onClick={() =>
                    handleListItemClick(bottomIndex, item.path, item.onClick)
                  }
